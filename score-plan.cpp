...
/*
Plan scorer overview (based on paper.tex and input-output-explanation.txt):
...
- At the end, the tool reports how many values are satisfied by the trace.
...
*/
static std::vector<int> evaluateFormulas(std::vector<NamedFormula>& formulas) {
    std::vector<int> values_out;
    values_out.reserve(formulas.size());
    for (NamedFormula& nf : formulas) {
        assert(nf.node != nullptr);
        values_out.push_back(nf.node->evaluate());
    }
    return values_out;
}
...
static ScoreSummary scorePlan(const std::vector<std::string>& plan) {
    std::vector<int> localVals;
    std::vector<int> globalVals;
    ...
    if (plan.empty()) {
        ...
        localVals = evaluateFormulas(local_formulas);
        globalVals = evaluateFormulas(global_formulas);
    } else {
        localVals = evaluateFormulas(local_formulas);
        globalVals = evaluateFormulas(global_formulas);
    ...
    summary.total = static_cast<int>(localVals.size() + globalVals.size());
    for (int v : localVals) {
        if (v == 1) summary.satisfied++;
    }
    for (int v : globalVals) {
        if (v == 1) summary.satisfied++;
    }
    summary.lost = summary.total - summary.satisfied;
    return summary;
}
...
int main(void){
 ...
 const ScoreSummary score = scorePlan(actions);
 std::cout << "satisfied=" << score.satisfied
           << " lost=" << score.lost
           << " total=" << score.total
           << " penalty=" << score.lost << std::endl;
 delete []lv_values;
 delete []gv_values;
 return 0;
}
