...
A benchmark-based empirical evaluation across multiple simulated robotic domains shows that the proposed locality-based method achieves the best solved-instance runtime profile, while exposing a clear speed--robustness tradeoff against broader baselines under fixed timeout budgets.
\end{abstract}
...
To complement the theoretical analysis, we perform an empirical comparison against brute-force and automata/progression baselines on systematic instances derived from the three examples in this paper, and report both runtime and completion behavior under fixed timeout budgets.
...
Table \ref{t:eval} summarizes success counts and runtime statistics on the full 190-instance suite. The locality-based planner solves 84 instances within the 10-second limit and times out on 106, with all 84 successful outputs validated. The automata/progression baseline solves 122 instances and times out on 68, with all 122 successful outputs validated. The brute-force baseline solves 96 instances and times out on 94, with all 96 successful outputs validated. Runtime statistics are computed over successful runs only: the locality planner has mean runtime 0.908s, median 0.06s, and maximum 8.86s; the automata/progression baseline has mean 2.743s, median 0.16s, and maximum 27.70s; and the brute-force baseline has mean 4.246s, median 0.61s, and maximum 28.48s.

To test whether the runtime advantage is statistically significant at the distribution level, we apply one-sided Mann--Whitney U tests with alternative hypothesis that locality runtimes are smaller than baseline runtimes. Using per-instance planner times (including timeout-capped values), we obtain $p=1.13\times 10^{-11}$ for locality versus brute force and $p=1.23\times 10^{-2}$ for locality versus automata/progression. These results indicate that locality is faster in distribution on this benchmark regime, but they do not imply higher completion robustness under the same timeout policy.
Because the locality planner's maximum successful runtime (8.86s) is below its 10-second timeout cap, the asymmetric timeout thresholds (10s for locality, 30s for baselines) do not artificially skew the Mann--Whitney rank ordering in favor of locality.
...
\midrule
Locality-based planner & 84/190 & 106 & 0.908 & 0.06 & 8.86 \\
Automata/progression baseline & 122/190 & 68 & 2.743 & 0.16 & 27.70 \\
Brute-force baseline & 96/190 & 94 & 4.246 & 0.61 & 28.48 \\
\bottomrule
\end{tabular}
\end{table}

To make the cross-example effect explicit, Table \ref{t:eval-families} reports success counts separately for the three benchmark families. The automata/progression baseline is strongest on Example 1 and Example 2, while the brute-force baseline is strongest on Example 3. The locality planner has the best solved-instance speed profile, but its completion rate degrades substantially on Example 2 and Example 3 under the fixed $L=3$ and timeout settings.
...
\midrule
Example 1 (linear traffic lights, 76 instances) & 56/76 & 65/76 & 44/76 \\
Example 2 (charging extension, 57 instances) & 16/57 & 36/57 & 17/57 \\
Example 3 (three-lane corridor, 57 instances) & 12/57 & 21/57 & 35/57 \\
\bottomrule
\end{tabular}
\end{table}
...
The baseline behavior is heterogeneous by family, which helps explain aggregate statistics. On Example 1, automata/progression has the highest completion and locality also remains competitive, while brute force starts to lose robustness. On Example 2, automata/progression clearly dominates completion and both locality and brute force time out on most instances. On Example 3, brute force becomes the most robust of the three, while automata/progression and locality both have high timeout rates under the fixed limits.

These results are partially consistent with the theory from Section \ref{pa}. The proposed method keeps only a bounded active window of local propositions, together with global propositions and compact temporal-status memory, and shifts this window left-to-right. This mechanism explains the low successful-run runtimes. At the same time, the finite timeout evaluation shows that this configuration is sensitive to hard instances and parameterization, so practical robustness does not automatically follow from asymptotic locality structure alone. The automata/progression baseline, by construction, explores product states that combine planning valuations with residual temporal formulas, which adds temporal-monitor overhead but appears to provide better search robustness on two of the three families. The brute-force baseline suffers from heavy tails in runtime yet remains comparatively robust on the third family.
...
The evaluation in Section \ref{eval} supports this claim in runtime terms on solved instances, while also revealing nontrivial robustness limits under fixed timeout budgets and parameter choices.
