...
\caption{Benchmark plots: runtime versus instance size $n$ (left; three instances per $n$) and runtime per case (right; one point per benchmark instance, 57 total, ordered deterministically by case index).}
\label{f3}
\end{center}
\end{figure}
...
Figure \ref{f3local} complements Figure \ref{f3} by isolating only the locality planner. This view confirms that runtime still increases with instance difficulty, but on a much smaller scale than for the baselines.
